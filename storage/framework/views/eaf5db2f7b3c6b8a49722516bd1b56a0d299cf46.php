    ...
    <div class="am-content">
        ...
        <script>
            ...
            $('#listTable tfoot th').each(function (index) {
                ...
                if (index == 2) {
                    $(this).html('<?php echo Form::select("request",$requests,'',array('class'=>'')); ?>');
                }
                if (index == 6) {
                    $(this).html('<?php echo Form::select("plans",$plans,'',array('class'=>'')); ?>');
                }
            ...
            var table = $('#listTable').DataTable({
                "bServerSide": true,
                "sAjaxSource": "<?php echo e(url('admin/workorderlistdata')); ?>",
                "sServerMethod": "POST",
                "iDisplayLength": 10,
                "searching": true,
                "processing": true,
            ...
            $('#listTable tbody').on('click', 'td.details-control', function () {
                ...
                if (row.child.isShown()) {
                    ...
                    $.ajax({
                        type: 'post',
                        url: "<?php echo e(url("admin/workordersublists")); ?>",
                        data: datastring,
                        dataType: "json",
            ...
            $(function ($) {
                $('body').on("click", '.remove', function () {
                    ...
                    $.confirm({
                        ...
                        text: "Do you want to delete this record ?",
                        confirm: function () {
                            var datastring = {workoderItemId:workoderItemId,"_token":"<?php echo csrf_token(); ?>"};
                            $.ajax({
                               type:"POST",
                                url:"<?php echo e(url('admin/deleteworkorderiem')); ?>",
                                data:datastring,
                                dataType:"JSON",
                                success:function(json){
                                  if(json.result)
