...
//
    ...
    public function AllEquipmentGrid($limit = 0, $offset = 0, $order_by = 'te.id', $direction = 'ASC', $cond = array(), $count = false, $likeArray = '', $dateField = '', $filter = array())
    { DB::enableQueryLog();
...
//    echo'<pre>';print_r($like);'</pre>';die;
        ...
        $query->join('tbl_brand as tb', 'tb.brand_id', '=', 'te.brand_id');
        $query->join('tbl_operations as tbo', 'tbo.id', '=', 'te.brand_operation');
        $query->join('tbl_channels as c', 'c.id', '=', 'te.channel','left');
        $query->join('tbl_volume as v', 'v.id', '=', 'te.volume','left');
        $query->join('tbl_units as u', 'u.id', '=', 'te.unit','left');
        $query->join('tbl_channel_numbers as cn', 'cn.id', '=', 'te.channel_number','left');
...
//        $input['modified_by'] = $userId;
        ...
        $delete = DB::table('tbl_equipment_model')->where('id', $id)->delete();
...
//    }
    ...
    public function dueequipments($limit = 0, $offset = 0, $cond = array())
    {
       ...
       // $query->limit(4);
        ...
        if(isset($cond['startNextDate']) && $cond['startNextDate'] != '')
        {
            $query->where('DE.next_due_date','>=', $cond['startNextDate']);
        }
        if(isset($cond['endNextDate']) && $cond['endNextDate'] != '')
        {
            $query->where('DE.next_due_date','<=', $cond['endNextDate']);
        }
    ...
    public function dueequipmentsByDate($limit = 0, $offset = 0, $cond = array())
    {
    ...
    public function viewAlldueEquipments($limit = 0, $offset = 0, $cond = array(),$count)
    {
        ...
        if (isset($cond['search']) && $cond['search'] != '') {
            ...
            $likeArrayFields = array('E.asset_no','E.serial_no','E.	pref_contact','E.location','EM.model_name','EM.model_description');
            if (!empty($likeArrayFields)) {
        ...
        $query->select('E.name', 'E.asset_no', 'E.serial_no', 'E.customer_id', 'E.pref_contact', 'E.pref_tel', 'E.pref_email', 'E.location', 'EM.model_description as model_name','F.name as call_frequency','E.id as equipment_id','DE.next_due_date','DE.last_cal_date','DE.id as due_equipment_id',DB::raw("date_format(str_to_date(DE.next_due_date, '%Y-%m-%d'), '%m-%Y') as d"),'SP.service_plan_name as service_plan','F.no_of_days');
...
//                }
                    $query->where(DB::raw("date_format(str_to_date(DE.next_due_date, '%Y-%m-%d'), '%m-%Y-%d')"),'=',$cond['monthYear']);
           }
            else
            {
    ...
    public function monthWiseDueEquipment($limit = 0, $offset = 0, $cond = array())
    {
        ...
        $query->join('tbl_customer_setups as CS', 'CS.customer_id', '=', 'E.customer_id','left');
        $query->join('tbl_frequency as F', 'F.id', '=', 'CS.cal_frequnecy','left');
        $query->join('tbl_frequency as Fr', 'Fr.id', '=', 'DE.frequency_id','left');
        $query->select('E.name', 'E.asset_no', 'E.serial_no', 'E.customer_id', 'E.pref_contact', 'E.pref_tel', 'E.pref_email', 'E.location', 'EM.model_description as model_name','F.name as call_frequency_value','F.no_of_days','E.id as equipment_id','DE.next_due_date','DE.last_cal_date','DE.id as due_equipment_id','DE.frequency_id','Fr.no_of_days as nOfDays',DB::raw("date_format(str_to_date(DE.next_due_date, '%Y-%m-%d'), '%Y-%m') as d"),DB::raw("date_format(str_to_date(DE.next_due_date, '%Y-%m-%d'), '%Y-%m-%d') as day"));
        $query->orderBy('DE.next_due_date', 'ASC');

        //previous code for due
        $query->where(DB::raw("date_format(str_to_date(DE.next_due_date, '%Y-%m-%d'), '%Y-%m-%d')"),'=',$cond['monthYear']);
        // $query->groupBy("d");
        // $query->limit(4);
...
//        $input['modified_by'] = $userId;
        ...
        $delete = DB::table('tbl_equipment')->where('id', $id)->delete();
...
//            $result = DB::table('tbl_manufacturer')->where('manufacturer_id', $id)->update($input);
    ...
    //check equipment exists r not

    function checkequipmentcombination($post)
    {
        ...
        if($post['volumeRange'])
        {
            $query->where([
                ...
                ['brand_id', '=', $post['brand']],
                ['brand_operation', '=', $post['operation']],
                ['volume', '=', $post['volume']],
