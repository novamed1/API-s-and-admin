...
{
    protected $table = 'tbl_work_order_items as oi';
    public function serviceSummary($limit = 0, $offset = 0,$cond = array(),  $direction = 'DESC',$order_by = 'oi.id',  $count = false, $likeArray = array('W.work_order_number','C.customer_name'), $dateField = '', $filter = array())
    {
        DB::enableQueryLog();
        ...
        $query->join('tbl_work_order as w','w.id','=','oi.work_order_id');
        ...
        $query->join('tbl_service_plan as sp','sp.id','=','w.plan_id');
        $query->join('tbl_technician as t','t.id','=','w.calibration_to','left');
    ...
    public function CalibrationDetails($limit = 0, $offset = 0, $cond = array(), $direction = 'DESC', $order_by = 'twoi.id', $count = false, $likeArray = array('twoi.work_order_id'), $dateField = '', $filter = array())
    {
        ...
        $query->join('tbl_users as tt', 'tt.id', '=', 'twoi.technician_review');
        $query->join('tbl_equipment as e', 'e.id', '=', 'de.equipment_id');
...
//                    DB::enableQueryLog();
    ...
    function workorderitemdetails($item_id)
    {
        ...
        $query = DB::table('tbl_work_order_items as oi')
            ...
            ->where('oi.id',$item_id)
            ->select('e.id','cn.id as channel_id','cn.channel_number','c.channel_name','o.operation_name','ri.comments','e.asset_no','e.serial_no','oi.id as work_order_items_id','de.last_cal_date','f.name','ri.test_plan_id','em.brand_operation','em.volume','em.channel_number as channel_number_id','em.id as emodel_id')->first();
        //print_r(DB::getQueryLog()); die;
        return $query;
    }
    ...
    function checklistItem($checklist_id)
    {
        ...
        $query = DB::table('tbl_checklist_item')
            ...
            ->where('parent_id',0)
            ->select('id','title','type','parent_id','checklist_id')->get();
        //print_r(DB::getQueryLog()); die;
        return $query;
    }
