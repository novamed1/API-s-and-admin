...
<table width="100%">
    <tr>
        ...
        <td width="40%" valign="top" style="margin-left: 40px;">
            ...
            <p style="font-size: 14px;font-weight: normal;color: #6f6f6f;line-height: 1.4;"><span style="color: #262222;">Quote Number</span> {{$qutationnumber}}</p>
            <p style="font-size: 14px;font-weight: normal;color: #6f6f6f;line-height: 1.4;"><span style="color: #262222;">Quote Date</span>&nbsp;&nbsp; {{date('m-d-Y')}}</p>
            <p style="font-size: 14px;font-weight: normal;color: #6f6f6f;line-height: 1.4;"><span style="color: #262222;">Terms</span>{{$data['paymentTerms']}}</p>
            <p style="font-size: 14px;font-weight: normal;color: #6f6f6f;line-height: 1.4;"><span style="color: #262222;">Quote Expires </span> {{date('m-d-Y', strtotime("+90 days"))}}</p>
    ...
    <tr>
        <td width="65%" valign="top">
            <p style="font-size: 14px;padding: 10px;font-weight: normal;color: #333333;line-height: 1.4;background-color: #eeeeee;border-radius: 7px;">
                ...
                {{(isset($data['customer']->customer_name)&&$data['customer']->customer_name)?$data['customer']->customer_name:''}} <br>
                Attn: {{$data['billing']->billing_contact}} <br>
                {{$data['billing']->address1}} <br>
                @if(isset($data['billing']->address2)&&$data['billing']->address2)
                {{$data['billing']->address2}} <br>
                @endif
                {{$data['billing']->city.','.$data['billing']->state.' '.$data['billing']->zip_code}}  <br>
                Tel: +1 {{$data['billing']->phone}} <br>

                {{$data['billing']->email}}</p>
        </td>
        <td width="35%" valign="top">
            <p style="font-size: 14px;font-weight: bold;color: #262222;line-height: 1.4;">Ship To</p>
            <p style="font-size: 14px;font-weight: normal;color: #262222;line-height: 1.4;">{{(isset($data['customer']->customer_name)&&$data['customer']->customer_name)?$data['customer']->customer_name:''}}</p>
            <p style="font-size: 14px;font-weight: normal;color: #262222;line-height: 1.4;">Attn: {{$data['prefContactName']}}</p>
            <?php
            ...
            <p style="font-size: 14px;font-weight: normal;color: #262222;line-height: 1.4;">{{$data['shipping']->address1}} </p>
            <p style="font-size: 14px;font-weight: normal;color: #262222;line-height: 1.4;">{{$data['shipping']->address2}} </p>
            <p style="font-size: 14px;font-weight: normal;color: #262222;line-height: 1.4;">{{$data['shipping']->city.','.$data['shipping']->state.','.$data['shipping']->zip_code}} </p>
            <p style="font-size: 14px;font-weight: normal;color: #262222;line-height: 1.4;"> Tel: +1 {{$data['shipping']->phone_num}}</p>
        </td>
    </tr>
</table>
...
<table width="100%" cellpadding="0" cellspacing="0" style="margin-top: 45px;">
    ...
    @if(count($items))
        <?php $i = 1; ?>
        <?php $p = 1 ?>
            @foreach($items as $key=>$row1)
                @if(count($row1['lineItems']))
                    @foreach($row1['lineItems'] as $row)
            <?php $p++ ?>
    ...
    <tr>
        ...
        <td style="border-top:solid 2px #000; font-size: 14px;padding:15px 10px 5px 0;text-align:right;font-weight: normal;color: #000;">$ {{number_format($totalCost,2)}}</td>
    </tr>

    <tr>
        <td colspan="5" style="font-size: 14px;padding:5px 0;text-align:right;font-weight: normal;color: #000;">Shipping & Handling</td>
        <td style="font-size: 14px;padding:5px 10px 5px 0;text-align:right;font-weight: normal;color: #000;">@if($shippingCharge)
    ...
    <tr>
        ...
        <td style="font-size: 14px;padding:10px 10px 5px 0;text-align:right;font-weight: normal;color: #000;">@if($GrandTotal)
