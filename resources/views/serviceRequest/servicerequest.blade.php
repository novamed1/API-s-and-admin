...
<style>
    ...
    {{--<style>--}}
        ...
        {{--}--}}
        {{--.onoffswitch-switch {--}}
            {{--display: block; width: 10px; margin: 3.5px;--}}
            {{--background: #FFFFFF;--}}
    ...
    <div class="am-content">
        ...
        <script>
            ...
            var table = $('#listTable').DataTable({
                "bServerSide": true,
                "sAjaxSource": "{{url('admin/servicerequestlistdata')}}",
                "sServerMethod": "POST",
                "iDisplayLength": 10,
                "searching": true,
                "processing": true,
            ...
            $('#listTable tbody').on('click', 'td.details-control', function () {
                ...
                if (row.child.isShown()) {
                    ...
                    $.ajax({
                        type: 'post',
                        url: "{{url("admin/servicerequestsublist")}}",
                        data: datastring,
                        dataType: "json",
    ...
    $(function ($) {
        ...
        <script>
                    ...
                    $('body').on('change', '.plan_change', function () {
                        ...
                        $.ajax({
                            type: 'post',
                            url: "{{url("admin/requestItemsPlanEdit")}}",
                            data: {request_item_id:request_item_id,plan_id:plan_id,"_token": "{!! csrf_token() !!}"},
                            dataType: "json",
                            success: function (json) {
                                if (json.result) {
                                    setTimeout(function(){
